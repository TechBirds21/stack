/*
  # Fix policy conflicts in database schema

  1. Tables
    - Ensures all required tables exist (users, properties, bookings, inquiries, etc.)
  ...
  3. Functions & Triggers
    - Creates custom ID generation functions
    - Sets up notification functions
    - Adds triggers for automated processes
*/
...
-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "System can create agent assignments" ON agent_inquiry_assignments;
DROP POLICY IF EXISTS "agent_assignments_select_policy" ON agent_inquiry_assignments;
DROP POLICY IF EXISTS "agent_assignments_update_policy" ON agent_inquiry_assignments;
DROP POLICY IF EXISTS "admin_assignments_all_policy" ON agent_inquiry_assignments;
...
DROP POLICY IF EXISTS "Users can create their own seller profile" ON seller_profiles;
...
-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "Agents can view their own earnings" ON earnings;
...
DROP POLICY IF EXISTS "Admin can manage all documents" ON documents;
...
-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "email_verification_tokens_select_policy" ON email_verification_tokens;
DROP POLICY IF EXISTS "Users can create their own verification tokens" ON email_verification_tokens;
...
-- Drop existing policies if they exist and recreate them
DROP POLICY IF EXISTS "admin_notification_queue_policy" ON notification_queue;
...
-- Drop existing policies if they exist and recreate them for system_counters
DROP POLICY IF EXISTS "Admin can view system counters" ON system_counters;
