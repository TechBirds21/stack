/*
  # Complete Admin Dashboard Data Migration
  ...
  2. Sample Data
    ...
    - Properties with proper relationships
    - Bookings with agent assignments
    - Inquiries with proper tracking
...
-- Handle email conflicts separately
INSERT INTO users (id, email, first_name, last_name, phone_number, user_type, status, verification_status, date_of_birth) VALUES
('a1111111-1111-1111-1111-111111111111', 'buyer1_alt@example.com', 'John', 'Smith', '+91 9876543210', 'buyer', 'active', 'verified', '1990-05-15'),
('a2222222-2222-2222-2222-222222222222', 'buyer2_alt@example.com', 'Sarah', 'Johnson', '+91 9876543211', 'buyer', 'active', 'verified', '1985-08-22'),
('a3333333-3333-3333-3333-333333333333', 'buyer3_alt@example.com', 'Mike', 'Davis', '+91 9876543212', 'buyer', 'active', 'pending', '1992-12-10')
ON CONFLICT (id) DO NOTHING;

-- Insert properties with conflict handling and proper UUIDs
...
-- Additional properties with proper UUIDs
...
('Modern 1BHK Apartment', 'Compact modern apartment perfect for young professionals.', 3200000, NULL, NULL, 'apartment', 1, 1, 800, 'Hitech City', 'Hyderabad', 'Telangana', '500081', 17.4483, 78.3915, ARRAY['https://images.pexels.com/photos/2404843/pexels-photo-2404843.jpeg'], ARRAY['Modern', 'IT Hub', 'Metro'], (SELECT id FROM users WHERE user_type = 'seller' LIMIT 1), 'active', false, true, 'SALE', NULL, 'Semi Furnished'),

('Beachfront Villa', 'Stunning villa right on the beach with private access.', NULL, 85000, 170000, 'villa', 5, 4, 4000, 'Rushikonda', 'Visakhapatnam', 'Andhra Pradesh', '530045', 17.7804, 83.3782, ARRAY['https://images.pexels.com/photos/1396122/pexels-photo-1396122.jpeg'], ARRAY['Beach Access', 'Private Pool', 'Garden'], (SELECT id FROM users WHERE user_type = 'seller' LIMIT 1 OFFSET 1), 'active', true, true, 'RENT', '2024-03-15', 'Fully Furnished'),

('Affordable 2BHK House', 'Great starter home for families on a budget.', 4500000, NULL, NULL, 'house', 2, 2, 1200, 'Uppal', 'Hyderabad', 'Telangana', '500039', 17.4065, 78.5691, ARRAY['https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg'], ARRAY['Family Friendly', 'Good Schools', 'Transport'], (SELECT id FROM users WHERE user_type = 'seller' LIMIT 1 OFFSET 2), 'active', false, true, 'SALE', NULL, 'Unfurnished');

-- Insert sample bookings with agent assignments (now that agent_id column exists)
INSERT INTO bookings (property_id, user_id, booking_date, booking_time, status, notes, agent_id) 
SELECT 
  p.id,
  u.id,
  '2024-01-25'::date,
...
-- Insert more bookings for better data
INSERT INTO bookings (property_id, user_id, booking_date, booking_time, status, notes, agent_id) 
SELECT 
  p.id,
  u.id,
  ...
  '15:00:00'::time,
  'pending',
  'Interested in villa for family',
...
SELECT 
  ...
  '+91 9876543219',
  'Hi, I am interested in this property. Can you share more details?',
  'new'
FROM properties p
...
INSERT INTO commissions (agent_id, property_id, commission_type, amount, percentage, status) VALUES
('77777777-7777-7777-7777-777777777777', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'sale', 150000, 1.0, 'approved'),
('88888888-8888-8888-8888-888888888888', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'sale', 75000, 1.0, 'paid'),
('77777777-7777-7777-7777-777777777777', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'rental', 2200, 10.0, 'pending'),
('99999999-9999-9999-9999-999999999999', 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee', 'rental', 4500, 10.0, 'approved');

-- Insert sample earnings data
INSERT INTO earnings (agent_id, month, year, total_commission, total_bookings, total_sales) VALUES
('77777777-7777-7777-7777-777777777777', 1, 2024, 152200, 3, 1),
('88888888-8888-8888-8888-888888888888', 1, 2024, 75000, 2, 1),
('99999999-9999-9999-9999-999999999999', 1, 2024, 4500, 1, 0),
('77777777-7777-7777-7777-777777777777', 12, 2023, 245000, 5, 2),
('88888888-8888-8888-8888-888888888888', 12, 2023, 180000, 4, 2);