/*
  ...
  1. New Tables
    - `documents` - Store uploaded documents (PDFs, images)
    - `notifications` - System notifications for admin
    - `system_counters` - Track unique ID counters
    - `agent_licenses` - Store agent license numbers

  2. Enhanced Tables
    - Add custom IDs to users and properties
    - Add agent license numbers
    - Add document references

  3. Functions
    - Generate unique IDs for users and properties
    - Generate agent license numbers
...
CREATE TABLE IF NOT EXISTS documents (
  ...
  entity_type text NOT NULL, -- 'user', 'property', 'agent'
  entity_id uuid NOT NULL,
  document_category text, -- 'profile', 'verification', 'property_image', 'property_document'
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type text NOT NULL, -- 'user_registration', 'inquiry', 'booking', 'verification'
  entity_type text, -- 'user', 'property', 'booking', 'inquiry'
  entity_id uuid,
  read boolean DEFAULT false,
  created_at timestamptz DEFAULT now()
);

-- Create system counters for unique IDs
...
-- Add custom ID columns to users table
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add custom ID column to properties table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'users' AND TG_OP = 'INSERT' THEN
    ...
    VALUES (
      ...
      'New ' || NEW.user_type || ' registered: ' || NEW.first_name || ' ' || NEW.last_name,
      'user_registration',
      'user',
      NEW.id
    );
  ...
  IF TG_TABLE_NAME = 'inquiries' AND TG_OP = 'INSERT' THEN
    ...
    VALUES (
      ...
      'New inquiry from ' || NEW.name || ' for property',
      'inquiry',
      'inquiry',
      NEW.id
    );
  ...
  IF TG_TABLE_NAME = 'bookings' AND TG_OP = 'INSERT' THEN
    ...
    VALUES (
      'New Tour Booking',
      'New tour booking scheduled for ' || NEW.booking_date,
      'booking',
      'booking',
      NEW.id
    );
...
('System Initialized', 'Admin system has been successfully initialized', 'system', 'system', now() - interval '1 hour'),
('New User Registrations', '3 new users registered today', 'user_registration', 'user', now() - interval '30 minutes'),
('Property Inquiries', '5 new property inquiries received', 'inquiry', 'inquiry', now() - interval '15 minutes'),
('Tour Bookings', '2 new tour bookings scheduled', 'booking', 'booking', now() - interval '5 minutes');