...
const CommunityBanners = () => {
  ...
  useEffect(() => {
    const filtered = mockBanners.filter(banner => 
      Object.values(banner).some(value => 
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Community Banners');
    XLSX.writeFile(wb, 'community_banners.xlsx');
  };
  ...
  const exportToCSV = () => {
    ...
    link.download = 'community_banners.csv';
    link.click();
  };
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden">
        <div className="bg-[#1E3A8A] p-6 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-white">Community Banners</h2>
          <button 
        ...
        <div className="p-6">
              ...
              <tbody>
                {filteredBanners.map((banner) => (
                  <tr key={banner.id} className="border-b">
                    ...
                    <td className="px-4 py-3">
                      <span className={banner.status === 'Active' ? 'text-[#22C55E]' : 'text-red-500'}>
                        {banner.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
          ...
          <div className="flex justify-between items-center mt-6">
            <div>
              Showing 1 to {filteredBanners.length} of {filteredBanners.length} entries
            </div>
            <div className="flex gap-2">
              <button 
