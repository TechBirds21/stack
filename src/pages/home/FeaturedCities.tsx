...
const FeaturedCities = () => {
  ...
  const [filteredCities, setFilteredCities] = useState<FeaturedCity[]>([]);

  const mockCities: FeaturedCity[] = [
    {
      id: 1,
      cityName: 'Visakhapatnam, Andhra Pradesh, India',
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(filteredCities);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Featured Cities');
    XLSX.writeFile(wb, 'featured_cities.xlsx');
  };
  ...
  const exportToCSV = () => {
    ...
    link.download = 'featured_cities.csv';
    link.click();
  };
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden">
        <div className="bg-[#1E3A8A] p-6 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-white">Featured Cities</h2>
          <button 
              ...
              <tbody>
                {filteredCities.map((city) => (
                  <tr key={city.id} className="border-b">
                    ...
                    <td className="px-4 py-3">{city.cityName}</td>
                    <td className="px-4 py-3">{city.displayName}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={city.status === 'Active' ? 'text-[#22C55E]' : 'text-red-500'}>
                        {city.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
