...
1. Email Verification System
   ...
   - Automatic email sending triggers
   - User verification status management

2. Agent Notification System
   - Inquiry assignment to agents
   - Agent acceptance/decline workflow
   - Notification queue management

3. Enhanced Notifications
   - Email verification notifications
   - Agent assignment notifications
   - Purchase/rental inquiry categorization
*/
...
BEGIN
  -- Get inquiry details
  SELECT i.*, p.* INTO inquiry_record, property_record
  FROM inquiries i
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.email_verified != NEW.email_verified AND NEW.email_verified = true THEN
    INSERT INTO notifications (title, message, type, entity_type, entity_id)
    VALUES (
      'Email Verified',
